...
import { t, titleAttr, I18N_CSS, I18N_HEAD, I18N_BODY } from './i18n.mjs'

/**
 * Analytics helper injected into every page <head> (multi-page site) and the
 * dashboard head. `window.__track(name, data)` forwards to Umami's `track`
 * when the script loaded, and is a silent no-op when it is blocked or absent
 * (ad blockers, offline, CSP) — it never throws and never blocks rendering.
 * Call sites must therefore treat it as best-effort and keep plain navigation.
 */
export const TRACK_HELPER = `<script>window.__track=function(name,data){try{if(window.umami&&typeof window.umami.track==='function')window.umami.track(name,data||{})}catch(e){}}</script>`
...
@media(max-width:720px){.brand small{display:none}.topbar .wrap{flex-wrap:wrap;height:auto;min-height:52px;padding:0 20px;row-gap:0}.menubtn{display:inline-flex;margin-left:auto}.nav{display:none;flex-basis:100%;flex-direction:column;align-items:stretch;gap:2px;padding:10px 0 14px;border-top:1px solid var(--line)}.topbar.open .nav{display:flex}.nav a{padding:9px 12px;font-size:14px}.nav .theme{align-self:flex-start;margin-top:8px}}
...
.sc-cols{display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:14px}
@media(max-width:860px){.sc-cols{grid-template-columns:minmax(0,1fr)}}
...
.sigbox{border:1px solid var(--line);border-radius:12px;background:var(--card);padding:14px 16px;margin:0 0 22px}
.sigbox>b{display:block;font-size:13px;margin-bottom:6px}
.sig{padding:9px 12px;border-radius:8px;font-size:12.5px;line-height:1.55;margin:6px 0;border-left:3px solid var(--line);background:var(--track)}
.sig.high{background:#fef9c3;border-color:#eab308;color:#713f12}
.sig.mid{background:color-mix(in srgb,#d97706 9%,transparent);border-color:var(--warn)}
.sig .sev{font:700 10.5px var(--mono);letter-spacing:.06em;text-transform:uppercase;margin-right:8px;opacity:.75}
:root[data-theme=dark] .sig.high{background:color-mix(in srgb,#eab308 15%,transparent);color:#fde68a}
@media(prefers-color-scheme:dark){:root:not([data-theme=light]) .sig.high{background:color-mix(in srgb,#eab308 15%,transparent);color:#fde68a}}
.subbar{border-bottom:1px solid var(--line);background:var(--bg)}
.subbar .wrap{display:flex;gap:2px;padding:0 28px;height:38px;align-items:center;overflow-x:auto;scrollbar-width:none}
.subbar .wrap::-webkit-scrollbar{display:none}
.subbar a{color:var(--mut);font-size:12.5px;padding:4px 10px;border-radius:7px;white-space:nowrap;flex:none}
.subbar a:hover{color:var(--ink);background:var(--track);text-decoration:none}
.subbar a.here{color:var(--ink);font-weight:650;background:var(--track)}
@media(max-width:720px){.subbar .wrap{padding:0 20px}}
...
const NAV = [
  ['', '首页', 'Home'],
  ['dashboard/', '插件', 'Plugins'],
  ['weekly/', '报告', 'Reports'],
  ['dynamics/', '动态', 'Dynamics'],
  ['data/', '资源', 'Resources'],
  ['about/', '关于', 'About'],
]

// 组内子导航：相关页面合并为一级入口后，二级在页面顶部条呈现
const GROUP_PLUGINS = [['dashboard/', '插件库', 'Directory'], ['scenarios/', '场景', 'Scenarios'], ['authors/', '作者', 'Authors']]
const GROUP_REPORTS = [['weekly/', '周报', 'Weekly'], ['insights/', '洞察', 'Insights']]
const GROUP_RESOURCES = [['data/', '开放数据', 'Open Data'], ['badge/', '徽章', 'Badge'], ['kit/', '插件', 'Kit']]
const SUBNAV = {
  'dashboard/': GROUP_PLUGINS, 'scenarios/': GROUP_PLUGINS, 'authors/': GROUP_PLUGINS,
  'weekly/': GROUP_REPORTS, 'insights/': GROUP_REPORTS,
  'data/': GROUP_RESOURCES, 'badge/': GROUP_RESOURCES, 'kit/': GROUP_RESOURCES,
}
// 组内页面在一级导航上的归属
const TOP_OF = { 'scenarios/': 'dashboard/', 'authors/': 'dashboard/', 'insights/': 'weekly/', 'badge/': 'data/', 'kit/': 'data/' }
 ...
 */
export function page({ title, desc = '', base = './', here = null, body = '', og = {}, titleEn = '' }) {
  const topHere = TOP_OF[here] ?? here
  const nav = NAV.map(([href, zh, en]) =>
    `<a href="${base}${href}"${topHere === href ? ' class="here"' : ''}>${t(zh, en)}</a>`).join('')
  const sub = SUBNAV[here]
    ? `<div class="subbar"><div class="wrap">${SUBNAV[here].map(([href, zh, en]) =>
        `<a href="${base}${href}"${here === href ? ' class="here"' : ''}>${t(zh, en)}</a>`).join('')}</div></div>`
    : ''
  const ogTags = [
...
${TRACK_HELPER}
${titleEn ? `<meta name="en-title" content="${escHtml(titleEn)} · DSH Insights">` : ''}
${ogTags}
...
<div class="topbar"><div class="wrap">
  ...
  <nav class="nav">${nav}<button class="theme lang" id="langBtn" ${titleAttr('切换到 English', 'Switch to 中文')}>EN</button><button class="theme" id="themeBtn" ${titleAttr('切换深浅色', 'Toggle dark / light')}><span class="t-moon">${icon('moon', 13)}</span><span class="t-sun">${icon('sun', 13)}</span></button><a href="https://github.com/ice5kysl/dsh-insights" target="_blank">GitHub ↗</a></nav>
</div></div>
${sub}
<main class="wrap">
<div class="content">
...
<footer>
  <span>${t('由', 'Built by')} <a href="https://github.com/ice5kysl/dsh-insights" target="_blank">DSH Insights</a> ${t('管线自动生成', 'pipeline, generated automatically')}</span>
  <span>${t('零依赖 · 启发式评估，非安全审计 · 数据 CC BY 4.0 / 代码 MIT', 'Zero-dependency · heuristic evaluation, not a security audit · data CC BY 4.0 / code MIT')} · <a href="${base}changelog/">${t('更新日志', 'Changelog')}</a></span>
</footer>
</main>
...
<script>(function(){var t=document.querySelector('.topbar'),b=document.getElementById('menuBtn');if(!t||!b)return;b.addEventListener('click',function(){var o=t.classList.toggle('open');b.setAttribute('aria-expanded',o?'true':'false')})})()</script>
${I18N_BODY}
</body>
</html>`
}
...
export function mdToHtml(md) {
  ...
  while (i < lines.length) {
    ...
    if (/^\s*(\d+\.|[-*])\s/.test(l)) {
      const isOrd = (s) => /^\s*\d+\./.test(s)
      const firstOrd = isOrd(l)
      const buf = []
      while (i < lines.length && /^\s*(\d+\.|[-*])\s/.test(lines[i]) && isOrd(lines[i]) === firstOrd) {
        const m = lines[i].match(/^(\s*)(?:\d+\.|[-*])\s+(.*)$/)
        buf.push({ ind: m[1].length, text: m[2] }); i++
      }
      const tag = firstOrd ? 'ol' : 'ul'
      const minInd = Math.min(...buf.map((x) => x.ind))
      let sub = false, liOpen = false
      html += `<${tag}>`
      for (const it of buf) {
        if (it.ind > minInd) {
          if (!sub) { html += `<${tag}>`; sub = true } // 嵌套列表落在父 li 内
          html += `<li>${inline(it.text)}</li>`
        } else {
          if (sub) { html += `</${tag}>`; sub = false }
          if (liOpen) html += '</li>'
          html += `<li>${inline(it.text)}`; liOpen = true
        }
      }
      if (sub) html += `</${tag}>`
      if (liOpen) html += '</li>'
