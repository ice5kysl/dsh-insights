...
/** db9 TIMESTAMPTZ 文本（'2026-09-11 14:34:33.318122+00'）→ ISO；无法解析返回 null。 */
export function toIso(t) {
  if (!t) return null
  const d = new Date(String(t).replace(' ', 'T').replace(/([+-]\d{2})$/, '$1:00'))
  return Number.isNaN(d.getTime()) ? null : d.toISOString()
}

/**
 * reports 表行 [sig, category, shell, plugin, created_at, source?] → crash-corpus signatures。
 *
 * 来源分离（source 列，见 dsh-crash-collect/scripts/migrations）：
 *   - count 只统计**真实用户上报**（source 缺省或非 'seed' 都算 organic，兼容旧调用）；
 *   - 冷启动种子（source='seed'，scripts/seed-corpus.mjs 灌入的实测语料）计入
 *     seededCount，绝不冒充「N 例上报」；
 *   - seeded=true 表示该签名目前**只有**种子、没有用户上报——消费端应换一种说法。
 *
 * 其余口径：按 count 倒序（并列按 seededCount 倒序，再按 sig 字典序）；
 * plugins/shells 按出现次数倒序、去 null/空串、各截断 10 个；category 取该 sig
 * 最常见值（并列取字典序小者）；firstSeen/lastSeen 优先取用户上报的 min/max，
 * 没有用户上报时回落到种子时间（种子-only 条目仍有实测日期）。
 */
export function aggregateCrashSignatures(rows) {
  const bump = (m, v) => { if (v !== null && v !== undefined && v !== '') m.set(v, (m.get(v) || 0) + 1) }
  const bySig = new Map()
  for (const r of rows || []) {
    const [sig, category, shell, plugin, createdAt] = r
    if (!sig) continue
    const seeded = r[5] === 'seed'
    let s = bySig.get(sig)
    if (!s) {
      s = {
        sig, count: 0, seededCount: 0,
        categories: new Map(), plugins: new Map(), shells: new Map(),
        firstSeen: null, lastSeen: null,     // 用户上报
        seedFirst: null, seedLast: null,     // 种子（回落用）
      }
      bySig.set(sig, s)
    }
    if (seeded) s.seededCount++
    else s.count++
    bump(s.categories, category)
    bump(s.plugins, plugin)
    bump(s.shells, shell)
    const t = toIso(createdAt)
    if (t) {
      if (seeded) {
        if (!s.seedFirst || t < s.seedFirst) s.seedFirst = t
        if (!s.seedLast || t > s.seedLast) s.seedLast = t
      } else {
        if (!s.firstSeen || t < s.firstSeen) s.firstSeen = t
        if (!s.lastSeen || t > s.lastSeen) s.lastSeen = t
      }
    }
  }
  const top = (m) => [...m.entries()].sort((a, b) => b[1] - a[1] || (a[0] < b[0] ? -1 : 1)).slice(0, 10).map(([v]) => v)
  return [...bySig.values()]
    .map((s) => ({
      sig: s.sig,
      category: top(s.categories)[0] ?? null,
      count: s.count,
      seededCount: s.seededCount,
      seeded: s.count === 0 && s.seededCount > 0,
      plugins: top(s.plugins),
      shells: top(s.shells),
      firstSeen: s.firstSeen ?? s.seedFirst,
      lastSeen: s.lastSeen ?? s.seedLast,
    }))
    .sort((a, b) => b.count - a.count || b.seededCount - a.seededCount || (a.sig < b.sig ? -1 : 1))
}

/** invalid.jsonl 行 → { key, reasons }：主键取 full_name，缺失回退 owner/repo；reason 归一为数组（当前实际结构是单字符串）。 */
export function invalidCandidateKey(r) {
  if (!r) return null
  const key = r.full_name || (r.owner && r.repo ? `${r.owner}/${r.repo}` : null)
  if (!key) return null
  return { key, reasons: [r.reason].filter(Boolean) }
}

/**
 * D4 回放台账（replay-history.jsonl）→ 按 (date, shell) 汇总。
 *
 * 从**完整台账**现算，所以是幂等的：重跑一次就会把该日该 shell 的汇总修正为真值，
 * 不需要增量维护。未知 verdict 只计入 total（不硬塞进任何桶，避免悄悄改口径）。
 */
export function aggregateReplayRuns(history) {
  const agg = new Map()
  for (const r of history || []) {
    if (!r?.repo || !r?.date) continue
    const k = `${r.date}|${r.shell ?? ''}`
    const a = agg.get(k) || { date: r.date, shell: r.shell ?? null, total: 0, ok: 0, broken: 0, degraded: 0, install_failed: 0 }
    a.total++
    const bucket = r.verdict === 'degraded' ? 'degraded' : r.verdict === 'install-failed' ? 'install_failed' : r.verdict
    if (bucket in a) a[bucket]++
    agg.set(k, a)
  }
  return [...agg.values()]
}
