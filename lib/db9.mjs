...
/**
 * reports 表行 [sig, category, shell, plugin, created_at] → crash-corpus signatures。
 * 按 count 倒序（并列按 sig 字典序）；plugins/shells 按出现次数倒序、去 null/空串、
 * 各截断 10 个；category 取该 sig 最常见值（并列取字典序小者）；firstSeen/lastSeen
 * 为 created_at 的 min/max（ISO）。
 */
export function aggregateCrashSignatures(rows) {
  ...
  for (const r of rows || []) {
    ...
    let s = bySig.get(sig)
    if (!s) {
      s = { sig, count: 0, categories: new Map(), plugins: new Map(), shells: new Map(), firstSeen: null, lastSeen: null }
      bySig.set(sig, s)
    }
    s.count++
    ...
    if (t && (!s.firstSeen || t < s.firstSeen)) s.firstSeen = t
    if (t && (!s.lastSeen || t > s.lastSeen)) s.lastSeen = t
  }
