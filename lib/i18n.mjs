...
/** head 内联：首帧前定语言（成对块靠 CSS 因此无闪烁；<html lang> 同步）。支持 ?lang=zh|en 显式指定（可分享链接），并持久化 */
export const I18N_HEAD = `<script>(function(){try{var q=new URLSearchParams(location.search).get('lang');if(q==='zh'||q==='en')localStorage.setItem('lang',q);var l=localStorage.getItem('lang');if(l!=='zh'&&l!=='en')l=((navigator.language||'zh')+'').toLowerCase().indexOf('zh')===0?'zh':'en';var d=document.documentElement;d.dataset.lang=l;d.lang=l==='en'?'en':'zh-CN'}catch(e){}})()</script>`
