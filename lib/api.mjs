...
async function raw(url, { retry = 3 } = {}) {
  // ⚠ GitHub 凭据只发给 GitHub。此前这里对**任何** URL 都挂 Authorization + GitHub
  // Accept，而 npmDoc 走的就是这条路径 —— 等于把 GITHUB_TOKEN 发给了 registry.npmjs.org
  // （第三方）。downloads.mjs / db9-sync 的 npmCreated 早就用裸 fetch 规避，此处补上守卫
  // （2026-09-17 修复）。
  const isGitHub = String(url).startsWith(GITHUB_API)
  const headers = { 'user-agent': 'dsh-insights' }
  if (isGitHub) {
...
async function npmDoc(name, { retry = 2 } = {}) {
  ...
  //
  // 裸 fetch（不走 raw）：registry.npmjs.org 是第三方，绝不能带 GitHub 凭据
  // （downloads.mjs / db9-sync 同款纪律）。瞬时失败重试，避免整行放弃更新。
  const url = `${NPM}/${String(name).replace(/^@/, '%40')}`
  let res
  try {
    res = await fetch(url, {
    ...
    if (retry > 0) { await sleep(1200 * (3 - retry)); return npmDoc(name, { retry: retry - 1 }) }
    return null
  }
  if (res.status === 404) return null
  if (!res.ok) {
    if (retry > 0 && (res.status === 429 || res.status >= 500)) { await sleep(1500 * (3 - retry)); return npmDoc(name, { retry: retry - 1 }) }
    return null
  }
  let d
