...
| file | 说明 |
|---|---|
| `data/plugins.jsonl` · `invalid.jsonl` | <!-- stats:begin -->**权威集 12,362** + 分桶 4,538（0 重复 · 硬门禁 · 2026-09-26 快照，校验滚动扩大中）<!-- stats:end --> |
...
| `data/crash-corpus.json` | 崩溃上报签名语料（聚合 db9 里 dsh-why --share 上报；`count` 只算用户上报，冷启动种子单列 `seededCount`；无 DB9_TOKEN 跳过） |
