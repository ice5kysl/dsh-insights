...
- 2026-09-10：**管线盲区——dynamics 漏掉 API 模型发布**：DeepSeek 上线多模态模型 deepseek-v4-flash-vision-exp / deepseek-v4.1-flash-expires-on-0910，我们的 dynamics（collect/dynamics.mjs）没抓到——它只盯 GitHub repo 元数据 + npm dist-tags，不探 DeepSeek API 的模型清单。待办：dynamics 增加模型清单探测（api.deepseek.com/models，CI 已有 DEEPSEEK_API_KEY），新模型出现应进 /dynamics/ 页与周报。
- 2026-09-17：**活跃度口径缺陷与 health-v6 更正（本轮最重要的发现）**——排查外联队列时发现：`metrics.active30/idleDays/ageDays/ageGate1` 由 `validate` 首次校验时用 `Date.now()` 算完即**永久冻结**，而 `collect/refresh.mjs`（其文件头明确宣称要修「写一次永不刷新导致的活跃度口径腐烂」）只刷新 `stars/pushed_at/topics` 等原始字段，**独独没有重算 score 真正读取的那几个派生值**（`score.mjs` 读 `r.metrics.active30`）。后果：① `activity.dormant`「超 30 天无提交」规则在 11,419 个插件里只有 **2 个**触发（应为 2,206）；② 站点「近 30 天活跃」虚报 **100%**（实为 78.2%）；③ 88% 的 metrics 相对实际落后 >1 天；④ 279 个 valid 行缺 `pushed_at`，活跃度被静默跳过。修复：新增 `lib/data.mjs#deriveActivity()`，评分/统计/CSV 导出统一现算；缺 `pushed_at` 记入 `missing`（不可知 ≠ 通过）。规则升 **health-v6**，回归测试 `tests/activity.test.mjs`（6 例），公告落 `data/changelog.json` v0.2.1 + 站点 /changelog/ + W36/W37 周报勘误。**实测影响比初判小**：S+A 1,139 → **1,050**（104 出 / 15 入，净 −89），均分 75.4；原因是多数停更插件本就被其它扣分压在 S/A 之下。**方法论教训**：初判曾用「受影响集合 × 掉档」外推，得出「S+A 腰斩至 558」的错误结论——**外推必须跑真实管线验证后才能对外说**（本次在对外之前用 `analyze` 实测纠正）。
- 2026-09-17：**外联渠道判读与队列换轴**——① #4399（awesome-dsh-plugin Discussions `Ideas`）发出 12 天 0 评论 0 upvote，该库 Discussions 活跃的是 `Plugins`/`Show and tell` 自发帖、`Ideas` 无人巡视，**判渠道无效、停止投入**（采纳路线改走「数据被引用」；重启前置条件 = health-v6 更正公开满一周，且只提「数据可引用」不提门槛）；② 官方 #5933 仍 1 upvote 0 评论；③ **第二批作者信（队列 11–20）整批弃用**——原名单全为 S(100) 满分插件、信件无实质发现，且名单建立在被冻结的活跃度指标上（2 位已实际停更 >30 天）。换轴为[缺陷驱动队列 B2](./OUTREACH-QUEUE-B2.md)：compat `never`/`broken-since` + 排 hostTransform + 近 30 天有提交 + 按周下载排序，命中 99 个（前 20 已列）。**发送前硬门禁**：指控崩溃类必须过真实运行时回放（vision-router 误报的教训），故 D4「实装 smoke 测试」最小切片成为该队列的前置工程。④ 首批战果回填：徽章 **5/10 落地**（workbench / image-gen / dream-skin（变基合入，此前漏计）/ genui / archived-chats），ego-browser 一封信促成作者发 0.8.4 修掉崩溃；#581 评论区 09-12 出现「回复吐槽都用 ai 回复吗😂」——**公开回复的 AI 腔已从内部担忧变成可观测风险**，外发文案需过「人味」检查。
- 2026-09-17：**D4 最小切片上线，当天即拦下 2 封误报信**——`pipeline/verify/replay.mjs` + `lib/cdp.mjs`：隔离 DSH_HOME（`.d4-sandbox/`，绝不碰用户 `~/.dsh`）→ 干净 baseline → `dsh plugin add` 真装 → 起真实 `dsh web` → headless Chromium 走 CDP 打开页面抓真实报错 → 与 baseline 差集归因 → `data/replay.json`。**零依赖**：Node ≥22 内置全局 WebSocket 手写 CDP（Target/Runtime/Log 三个域），浏览器复用 Playwright 已下载的缓存，不引 playwright 包。踩坑三条已写入文档：① 必须 `--no-sandbox`（否则 Chrome 写 Crashpad 被沙箱挡掉 → ws 一两秒后 1006 断开，表现为「连上但所有命令超时」）② 必须 `--remote-allow-origins=*` ③ **必须清残留写锁**——dsh 的 `$DSH_HOME/.credentials.yaml.lock` 被 SIGKILL 的 shell 不释放，残留锁会让下一个 shell 报 `atomic-write: timed out waiting for the writer lock`，表现为**「连对照组都 broken」的环境性假阳性**（首轮批量就踩了，已加清锁 + 命中该错重试一次）。**首次抽样结果（shell 0.1.5-rc.1，对照 dsh-insights-kit=ok）**：B2 队列前 5 个里 **2 个静态说 broken 但实跑完全正常**（WSL043/dsh-chat-manager、hellodigua/dsh-share）——**静态口径 40% 假阳性**，若直接发信就是两封新的 vision-router 式误报；另 3 个真 broken，且根因**全在 host 侧**（`@deepseek-ai/dsh-settings` 的 `settingsNamespace` 导出缺失 ×2、`cannot get property "webServer" without inject`），静态分析只看 client bundle 的 require，**连「为什么」都指错了**。结论：外联信件必须引用回放给出的根因；队列门禁改为「`data/replay.json` 里 `verdict === 'broken'` 才允许发信」。里程碑状态：D4 由 ⬜ 未做 → 🟡 最小切片上线（[docs/D4-REPLAY.md](./D4-REPLAY.md)）；待办 = CI 矩阵化（`.github/workflows/replay.yml` 已建，workflow_dispatch 手动，**尚未在 CI 实跑过**）+ 回放结论叠进 `/p/` 与 `/data/` 展示。
- 2026-09-17：**D4 升到 Tier 1：一次真实运行采齐多份事实；并挖出第二个冻结数据缺陷**——回放不再只回答「崩没崩」，同一轮顺手采：`egress[]`（CDP `Network` 域，只留 host、剥路径 query、排除自身与 localhost → 把 safety 从源码启发式升级为运行时观测）、`shot`（CDP 截图，1280×800 webp q70 ≈11KB/张，**只对 grade∈{S,A} 且回放 ok 出图**）、`hostBootMs`、`warnings[]`、`installed`（依赖/peer/install 脚本/license/engines.dsh）。新增 append-only 时间序列 `data/replay-history.jsonl`（同 `(date,shell,repo)` 覆盖、历史保留）→ 可派生「修好了」事件（作者修复榜的燃料）与崩溃率趋势。踩坑补一条：**必须先关「Internal Testing Notice」首启弹窗**，否则截图全是同一张公告图、插件面板被盖住（截图能力等于白做）。**最终抽样（8 目标 / shell 0.1.5-rc.1）**：静态说 broken 的 5 个里 **3 个实跑正常 → 假阳性率 60%**；真 broken 2 个（dsh-at-file@0.6.3、dsh-restart@0.1.2），根因全在 **host 侧**（`@deepseek-ai/dsh-settings` 的 `settingsNamespace` 导出缺失），静态只看 client bundle 的 require，连「为什么」都指错。附带 dogfood 收获：`dsh-workspace-kit@0.1.3` 实跑 `degraded`（client `require("@deepseek-ai/dsh-client-runtime/client") missed the module table`，正是我们提醒别人的那一类），0.1.11 已恢复。
- 2026-09-17：**第二个冻结数据缺陷——权威集 `npm` 数据不刷新**（回放装包时暴露）。`validate` 首次校验算出的 `r.npm`（latest/versions/latestTime）此后永不更新，`refresh.mjs` 只刷新 stars/pushed_at/archived/fork/topics。抽样重探 40 个已发布插件：**约 25% 的权威集版本号已过期**；本批 8 个目标 6 个 drift（insights-kit corpus 0.5.0 vs npm 0.10.2）。**它直接制造过假阳性**：Tkingxiao/dsh-any-background 在 corpus 记的 0.2.2 上确实崩，而 npm 真实 latest 0.2.9 实跑正常——按静态口径发信就会去教一个已经修好的作者做事。影响面：`npm.version-drift` 扣分、站点 npm 版本展示、队列表 `pkg@version`、回放验到的包。**装置侧已修**（`resolveTargets` 现探 registry 取真实 latest + 记 `versionDrift`），**权威集本身待决策**（与 health-v6 同类，属会改分的口径变更，且需评估 4,765 个包的 registry 重探成本）。
- 2026-09-17：**health-v7：npm 数据新鲜度修复（第二类冻结缺陷落地）+ 一个凭据外泄修复**——`refresh.mjs` 只刷 stars/pushed_at 等字段，不重探 npm，`r.npm` 停在入库那天。新增 `pipeline/collect/npm-refresh.mjs`（进 daily + monday，analyze 之前）日更：极廉价的 `/-/package/<pkg>/dist-tags` 探测 + **仅 latest 变化时**才拉完整 packument；无实质变化不写文件（避免每天全文件 diff）。**全量重跑（11,419 行 / 7 分钟）**：新版本 1,321 · 首次发布 371 · 已下架 39 · 无变化 9,686。口径影响：`npm.version-drift` 扣分 **1,369 → 2,642**（旧数据把 drift 藏住了）、`npm.unpublished` 6,654 → 6,291、npm 发布率 **41.7% → 44.9%**、S+A **1,050 → 810**、632 个插件档位变化、均分 75.4 → 75.1。**npm 端点怪癖（已记档）**：不存在的作用域包在 `/-/package/<spec>/dist-tags` 上返回 **401 而非 404**（隐藏私有包是否存在），按瞬时失败处理会每次都白重试 —— 必须按 missing。**同批修掉凭据外泄**：`lib/api.mjs` 的 `raw()` 把 GitHub token 与 GitHub Accept 头挂到**任何** URL，而 `npmDoc` 走该路径 → 每次 npm 探测都在把 GITHUB_TOKEN 发往 registry.npmjs.org（第三方）；现限定只对 GitHub 域加凭据，`npmDoc` 改裸 fetch。回归测试 `tests/npm-refresh.test.mjs`（7 例）。
- 2026-09-17：**D4 回放数据入库 db9**——新增两张表：`plugin_replay`（每 (repo,shell,date) 一行，detail 存当日完整观测：shellError / egress / installed / page / shot 路径）与 `replay_runs`（按 (date,shell) 汇总 total/ok/broken/degraded/install_failed）。台账源取 `replay-history.jsonl`（append-only 时间序列），汇总用 `lib/db9.mjs#aggregateReplayRuns` 从完整台账**现算**（幂等，重跑即修正）。这样「修好了」事件与崩溃率趋势可直接 SQL 查询；`data/` 仍是唯一事实来源，db9 为旁路镜像。注：本机未配置 `DB9_TOKEN`，**只验证到跳过路径**，实际建表/写入需在 CI 或配了 token 的环境跑一次确认。
- 2026-09-21：**W38 首期自动周报暴露的三个缺陷，当日修复**——W38 是第一期由 db9 cron 自动触发的周报（发布正常：站点 /weekly/、feed.xml、db9 weekly_letters 均已接力）。但盘点时发现三个问题：① **同一天跑了两次 monday**（run 35546711259 @00:09 与 35550630452 @01:33，各约 83 分钟）——根因是**排队的 run 携带的是入队时的 head_sha 而非启动时的**，`data/last-daily.txt` 靠 run 末尾提交回仓库，于是第二个 run checkout 到的是旧状态、去重失效；代价是双倍 CI 与双倍 `insights` 的 LLM 费用。修法：mode 判定前 `git fetch --depth=1 origin main && git reset --hard origin/main` 对齐远端，并加一道**按交付物**的幂等守卫（该 ISO 周的 `data/weekly/<WK>-*.md` 已存在 → monday 降级 hourly，不受标记文件竞态影响）。② **周报「npm 版本滞后（仓库领先于发布）」标题与数据方向相反**——生成器标题写死，而取数口径是 `npm.latest !== r.version`（任意不等），实际列出的 5 条全是 npm 领先仓库。修法：新增 `lib/version.mjs#cmpVersion`（粗粒度、非完整 semver，已声明边界），analyze 产出 `npmRepoAheadTop` / `npmRegistryAheadTop` 两个方向，周报分两节渲染——**「仓库领先、尚未发布」才是可行动的**（如 Aegis 仓库 2.9.6 vs npm 0.1.0），旧标题把这批信息整个埋了。③ **同一个 npm 包被列两次**——`dsh-market/dsh-market` 与 `KokuYu-sysu/dsh-market-desktop` 同指包 `dshmarket`，周下载 89,707 在两行各出现一次，读者以为 18 万。修法：`dlTop` 按 pkgName 去重，留星数最高的仓库作代表（enrich 行新增 `pkgName` 字段）。回归测试 `tests/version.test.mjs`（7 例）。
- 2026-09-21：**顺带核出一个被误读的指标**——30 天活跃率从 09-16 的 78.2% 掉到 57.4%，初看像生态恶化，实为**创建潮过期的机械结果**：30 天线在 09-16 位于 08-17、现在滑到 08-22，而「最后 push 恰好落在这 5 天窗口内」的插件有 **2,959 个**（下一段 08-10~08-17 还有 1,718 个）——边界一滑过去它们整批由"活跃"翻为"停更"。结论：**30 天活跃率是被创建潮结构污染的滞后指标，不可单独解读**，必须结合创建时间分布。此洞察只有在 health-v6 修完之后才可见（修前该值恒为 100%）。
