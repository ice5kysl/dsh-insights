...
> ✅ **下表 `npm` 列的版本号是 2026-09-17 修复前的快照**（权威集 npm 数据当时被冻结，抽样约 25% 已过期）。health-v7 已修：新增日更的 `npm-refresh` 阶段（全量重跑：新版本 1,321 / 首次发布 371 / 已下架 39），回放装置也改为**现探 registry 取真实 latest**。**下表版本号仅作历史记录，发信前一律以 `data/replay.json` 为准。**
