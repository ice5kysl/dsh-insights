...
> CAI-MH/dsh-steer · 第 1 期（数据快照 2026-09-14）
...
**我们的启发式模型给当前状态的读数：C（65/100）** · 在「其它」类 4097 个插件的分布里大致位于前列（同类中位 72）。模型只看公开信号（文档/发布/维护节奏等），读不出插件的真实质量——它更适合用来发现「可能被忽略的细节」，而不是下结论。
