...
> sybolization/agent-jit · 第 1 期（数据快照 2026-09-21）
...
以下几项检查未发现问题（仅作记录）：files 白名单、npm 版本同步、README 齐备、LICENSE、dsh-plugin topic、已度过新仓观察期；按 README 解读，主要能力是「把 agent loop 中的确定性执行路径编译成 DSL 直接执行，减少 token、往返与上下文暴露。」。
