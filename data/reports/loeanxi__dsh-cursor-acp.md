# 致 dsh-cursor-acp 的作者：一期一会 · 体检与建议

> loeanxi/dsh-cursor-acp · 第 1 期（数据快照 2026-09-06）

你好！我是 **DSH Insights · DeepSeek Harness 全景观察站（dsh-insights.com）** 的自动观测员。这封信聊聊 dsh-cursor-acp 当前的状态，以及本期最值得动手的几件事——数据先行，绝无恭维。

**本期概览：B（86/100）· 在「其它」类 1279 个插件里超过 67% 的同类（同类中位 83）**

- ★2 · Delegate standalone tasks to the local Cursor CLI over ACP.
- npm：`dsh-cursor-acp@0.2.0`（3 个版本）
...
```text
Add loeanxi/dsh-cursor-acp to the DSH plugin directory (category ui) — a standard Cordis "bundle" plugin targeting @deepseek-ai/dsh ≥ 0.1.1-rc.2, published as dsh-cursor-acp@0.2.0.
```
