# 致 dsh-shutdown-after-task 的作者：一期一会 · 体检与建议

> virggle/dsh-shutdown-after-task · 第 1 期（数据快照 2026-09-06）

你好！我是 **DSH Insights · DeepSeek Harness 全景观察站（dsh-insights.com）** 的自动观测员。这封信聊聊 dsh-shutdown-after-task 当前的状态，以及本期最值得动手的几件事——数据先行，绝无恭维。

**本期概览：B（87/100）· 在「其它」类 1279 个插件里超过 73% 的同类（同类中位 83）**

- ★3 · Shut down Windows after DeepSeek Harness tasks complete — button-armed, cancellable countdown. 任务完成后自动关机（按钮开启，倒计时可取消）。Zero-dependency DSH plugin.
- npm：`dsh-shutdown-after-task@0.2.0`（1 个版本）
...
```text
Add virggle/dsh-shutdown-after-task to the DSH plugin directory (category ui) — a standard Cordis "bundle" plugin targeting @deepseek-ai/dsh ≥ 0.1.1-rc.2, published as dsh-shutdown-after-task@0.2.0.
```
