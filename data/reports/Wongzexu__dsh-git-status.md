...
> Wongzexu/dsh-git-status · 第 1 期（数据快照 2026-09-06）
...
- ★4 · Git status (Git Graph) plugin for DSH: commit DAG lane graph + uncommitted changes/stash + inline diffs + branch operations. DSH 插件：Git 状态浮窗（泳道图/未提交/stash/diff/
- npm：`@wongzexu/dsh-git-status@1.0.0`（4 个版本）
...
```text
Add Wongzexu/dsh-git-status to the DSH plugin directory (category ui) — a standard Cordis "bundle" plugin targeting @deepseek-ai/dsh ≥ 0.1.1-rc.2, published as @wongzexu/dsh-git-status@1.0.0.
```
