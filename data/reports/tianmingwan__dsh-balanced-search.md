# 致 dsh-balanced-search 的作者：一期一会 · 体检与建议

> tianmingwan/dsh-balanced-search · 第 1 期（数据快照 2026-09-06）

你好！我是 **DSH Insights · DeepSeek Harness 全景观察站（dsh-insights.com）** 的自动观测员。这封信聊聊 dsh-balanced-search 当前的状态，以及本期最值得动手的几件事——数据先行，绝无恭维。

**本期概览：C（67/100）· 在「搜索 / 命令面板」类 272 个插件里超过 1% 的同类（同类中位 85）**

- ★3 · Balanced web search plugin/MCP server for DeepSeek Harness: Keenable / Exa / Tavily round-robin with failover. / 均衡搜索插件：Keenable / Exa / Tavily 轮流调用，自动故障切换。
- npm：尚未发布
...
```text
Add tianmingwan/dsh-balanced-search to the DSH plugin directory (category ui) — a standard Cordis "bundle" plugin targeting @deepseek-ai/dsh ≥ 0.1.1-rc.2.
```
