...
> Minokun/dsh-quota · 第 1 期（数据快照 2026-09-06）
...
**做得好的**：client 导出齐备、产物布局规范、files 白名单、npm 已发布、npm 版本同步、README 齐备、dsh-plugin topic、已度过新仓观察期、近期活跃 等检查全部通过；按 README 解读，主要能力是「从仓库名推断提供配额管理能力，缺乏详细说明。」。这些是你的基本盘，保持即可。
...
```text
Add Minokun/dsh-quota to the DSH plugin directory (category ui) — a standard Cordis "bundle" plugin targeting @deepseek-ai/dsh ≥ 0.1.1-rc.2, published as dsh-quota@0.9.0.
```
