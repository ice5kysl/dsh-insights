...
> LnsiAxe/dsh-web-restart · 第 1 期（数据快照 2026-09-21）
...
**我们的启发式模型给当前状态的读数：C（60/100）** · 在「其它」类 4315 个插件的分布里大致位于前列（同类中位 71）。模型只看公开信号（文档/发布/维护节奏等），读不出插件的真实质量——它更适合用来发现「可能被忽略的细节」，而不是下结论。

- ★0 · DSH bundle: ensure/rotate the DeepSeek Harness web server (detached via WMI) — load to start it
- npm：`@lnsiaxe/dsh-web-restart@0.1.0`（1 个版本）
- 周下载：**17**
- 最近 push 2026-08-14
