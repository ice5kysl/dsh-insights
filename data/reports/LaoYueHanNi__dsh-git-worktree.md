...
> LaoYueHanNi/dsh-git-worktree · 第 1 期（数据快照 2026-09-06）
...
**做得好的**：client 导出齐备、产物布局规范、files 白名单、npm 版本同步、README 齐备、LICENSE、dsh-plugin topic、已度过新仓观察期、近期活跃 等检查全部通过；按 README 解读，主要能力是「在 Web 界面切换分支，并通过 git worktree 实现隔离的 DSH 插件。」。这些是你的基本盘，保持即可。
...
```text
Add LaoYueHanNi/dsh-git-worktree to the DSH plugin directory (category ui) — a standard Cordis "bundle" plugin targeting @deepseek-ai/dsh ≥ 0.1.1-rc.2.
```
