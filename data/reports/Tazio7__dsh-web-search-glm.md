...
> Tazio7/dsh-web-search-glm · 第 1 期（数据快照 2026-09-21）
...
**我们的启发式模型给当前状态的读数：D（57/100）** · 在「搜索 / 命令面板」类 657 个插件的分布里大致位于前列（同类中位 78）。模型只看公开信号（文档/发布/维护节奏等），读不出插件的真实质量——它更适合用来发现「可能被忽略的细节」，而不是下结论。
