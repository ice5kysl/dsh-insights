...
> GooDAnDReaDY/dsh-vision-bridge · 第 1 期（数据快照 2026-09-06）
...
- ★1 · Universal vision bridge for DeepSeek Harness: multimodal attachments with text-only chat models
- npm：尚未发布
- 最近 push 2026-09-04 · 收录：awesome-dsh-plugin ✅ · imsai —

**做得好的**：files 白名单、npm 版本同步、README 齐备、LICENSE、dsh-plugin topic、已度过新仓观察期、近期活跃 等检查全部通过；按 README 解读，主要能力是「通用视觉桥接，让纯文本聊天模型也能处理多模态附件。」。这些是你的基本盘，保持即可。
...
```text
Add GooDAnDReaDY/dsh-vision-bridge to the DSH plugin directory (category ui) — a standard Cordis "bundle" plugin targeting @deepseek-ai/dsh ≥ 0.1.1-rc.2.
```
