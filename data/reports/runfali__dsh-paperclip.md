...
> runfali/dsh-paperclip · 第 1 期（数据快照 2026-09-06）
...
**做得好的**：files 白名单、npm 已发布、README 齐备、中文/双语文档、LICENSE、dsh-plugin topic、已度过新仓观察期、近期活跃 等检查全部通过；按 README 解读，主要能力是「零侵入式bundle插件：输入框上传、待发送文件浮层、read_document多格式分页读取、通用开关。」。这些是你的基本盘，保持即可。
...
```text
Add runfali/dsh-paperclip to the DSH plugin directory (category ui) — a standard Cordis "bundle" plugin targeting @deepseek-ai/dsh ≥ 0.1.1-rc.2, published as dsh-paperclip@0.2.5.
```
